...
from GPNN.NN import get_NN_indices_low_memory, get_NN_indices
...
class PytorchNNLowMemory:
    ...
    def search(self, queries):
        return get_NN_indices_low_memory(queries.to(self.device), self.index_vectors, self.alpha, self.batch_size, self.metric).cpu()
